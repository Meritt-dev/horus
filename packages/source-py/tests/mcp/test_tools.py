...
def mock_storage():
    ...
    storage.get_heritage.return_value = []
    storage.get_file_imports.return_value = []
    storage.get_file_importers.return_value = []
    storage.get_dead_code_symbols.return_value = []
    storage.get_symbols_in_file.return_value = []
    storage.get_file_coupling.return_value = []
    storage.get_community_members.return_value = []
    storage.get_communities_summary.return_value = []
    storage.get_cross_community_processes.return_value = []
    storage.get_node_communities.return_value = []
    storage.get_node_processes.return_value = []
    storage.get_file_community_counts.return_value = []
...
def _sym(
    ...
    **flags,
    ...
    """Build a GraphNode for stubbing ``get_symbols_in_file``."""
    return GraphNode(
        id=f"{label.value}:{file_path}:{name}",
        label=label,
        name=name,
        file_path=file_path,
...
class TestHandleContext:
    ...
    def test_heritage_shown(self, mock_storage):
        ...
        mock_storage.get_heritage.return_value = [("User", "src/models.py", "extends")]
    ...
    def test_imported_by_shown(self, mock_storage):
        mock_storage.get_file_importers.return_value = [
...
class TestHandleDeadCode:
    ...
    def test_dead_code_query_exception(self, mock_storage):
...
class TestHandleDetectChanges:
    def test_parses_diff(self, mock_storage):
        # handle_detect_changes uses get_symbols_in_file() to find symbols in
        # the changed file.
        mock_storage.get_symbols_in_file.return_value = [
            _sym("validate", "src/auth.py", 10, 30),
        ]
...
class TestHandleCypher:
    def test_returns_results(self, mock_storage):
        ...
        result = handle_cypher(
            mock_storage, "SELECT name, file_path, start_line FROM nodes"
        )
...
class TestResources:
    ...
    def test_get_overview(self, mock_storage):
        ...
        assert "Horus Codebase Overview" in result
...
class TestHandleCoupling:
    def test_returns_coupled_files(self, mock_storage):
        mock_storage.get_file_coupling.return_value = [
            ("src/auth/session.py", 0.85, 12), ("src/tests/test_login.py", 0.72, 9)
        ]
        mock_storage.get_file_imports.return_value = ["src/auth/session.py"]
    ...
    def test_min_strength_filter(self, mock_storage):
        mock_storage.get_file_coupling.return_value = [("src/weak.py", 0.2, 2)]
...
class TestHandleCommunities:
    def test_list_all_communities(self, mock_storage):
        mock_storage.get_communities_summary.return_value = [
    ...
    def test_drill_into_community(self, mock_storage):
        mock_storage.get_community_members.return_value = [
            ("run_pipeline", "function", "src/pipeline.py", 45, True, False),
            ("SqliteBackend", "class", "src/storage.py", 10, False, True),
        ]
        ...
        assert "SqliteBackend" in result
...
class TestHandleExplain:
    def test_basic_explanation(self, mock_storage):
        ...
        mock_storage.get_node_communities.return_value = ["ingestion+storage"]
        mock_storage.get_node_processes.return_value = ["run → walk → parse"]
...
class TestHandleReviewRisk:
    def test_basic_risk_assessment(self, mock_storage):
        mock_storage.get_symbols_in_file.return_value = [
            _sym("validate", "src/auth.py", 10, 30, is_entry_point=False),
        ]
        mock_storage.get_file_coupling.return_value = [("src/tests/test_auth.py", 0.82, 0)]
        mock_storage.get_node_communities.return_value = ["auth+security"]
        mock_storage.traverse_with_depth.return_value = [
    ...
    def test_flags_missing_cochange_files(self, mock_storage):
        ...
        mock_storage.get_file_coupling.return_value = [("src/tests/test_auth.py", 0.82, 0)]
        mock_storage.get_node_communities.return_value = ["auth"]
        mock_storage.traverse_with_depth.return_value = []
        result = handle_review_risk(mock_storage, SAMPLE_DIFF)
...
class TestHandleFileContext:
    def test_full_file_context(self, mock_storage):
        mock_storage.get_symbols_in_file.return_value = [
            _sym("handle_query", "src/mcp/tools.py", 170, is_entry_point=True),
            _sym("handle_context", "src/mcp/tools.py", 197),
        ]
        mock_storage.get_file_imports.return_value = [
            "src/storage/base.py", "src/search/hybrid.py"
        ]
        mock_storage.get_file_importers.return_value = ["src/mcp/server.py"]
        mock_storage.get_file_coupling.return_value = [("tests/mcp/test_tools.py", 0.85, 12)]
        mock_storage.get_file_community_counts.return_value = [("mcp+server", 2)]
    ...
    def test_file_with_dead_code(self, mock_storage):
        mock_storage.get_symbols_in_file.return_value = [
            _sym("old_func", "src/old.py", 45, is_dead=True),
        ]
