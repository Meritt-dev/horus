...
from horus_source.core.graph.model import GraphNode, NodeLabel, RelType, generate_id
from horus_source.core.ingestion.parser_phase import (
    FileParseData,
    get_parser,
    parse_file,
    process_parsing,
...
class TestParseFilePython:
    ...
    def test_oversized_file_skips_deep_parse(self) -> None:
        # Pathologically large files (generated SDKs/GraphQL, minified bundles,
        # vendored blobs) produce huge ASTs that OOM the indexer. parse_file must
        # skip symbol extraction for them rather than blow up memory.
        from horus_source.core.ingestion.parser_phase import _MAX_PARSE_BYTES

        huge = ("class Foo: pass\n" * (_MAX_PARSE_BYTES // 10))  # well over the cap
        data = parse_file("src/generated/huge.py", huge, "python")

        assert isinstance(data, FileParseData)
        assert data.file_path == "src/generated/huge.py"
        # No symbols extracted — deep parse was skipped.
        assert data.parse_result.symbols == []
...
class TestProcessParsingHandlesError:
    def test_process_parsing_handles_error(
        ...
        graph.add_node(
            GraphNode(
                id=generate_id(NodeLabel.FILE, "src/bad.cob"),
                label=NodeLabel.FILE,
                name="bad.cob",
                file_path="src/bad.cob",
                language="cobol",
            )
        )
        files = [_make_file_entry("src/bad.cob", "fn main() {}", "cobol")]
        result = process_parsing(files, graph)
    ...
    def test_error_does_not_affect_other_files(
        ...
        files = [
            _make_file_entry("src/bad.cob", "fn main() {}", "cobol"),
            _make_file_entry("src/utils.py", PYTHON_CODE, "python"),
        ]
        result = process_parsing(files, graph)
