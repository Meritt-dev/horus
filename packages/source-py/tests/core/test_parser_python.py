...
from horus_source.core.parsers.base import ParseResult
from horus_source.core.parsers.python_lang import PythonParser
...
class TestParseDecoratorArgs:
    """Decorator string-literal arguments are captured for signal->symbol seeding."""

    def test_single_string_arg(self, parser: PythonParser) -> None:
        code = "@processor('MANAGE_SALES')\ndef handle():\n    pass\n"
        result = parser.parse(code, "test.py")
        assert result.symbols[0].decorator_args == ["MANAGE_SALES"]

    def test_route_arg_keeps_first_positional_only(self, parser: PythonParser) -> None:
        # Keyword arguments (methods=[...]) are config, not the wiring name — only the
        # positional route literal is captured.
        code = "@app.route('/orders', methods=['GET'])\ndef orders():\n    pass\n"
        result = parser.parse(code, "test.py")
        assert result.symbols[0].decorator_args == ["/orders"]

    def test_fstring_arg_is_not_captured(self, parser: PythonParser) -> None:
        # An interpolated f-string has no statically-known value, so it is not a
        # deterministic wiring name.
        code = '@app.get(f"/users/{x}")\ndef users():\n    pass\n'
        result = parser.parse(code, "test.py")
        assert result.symbols[0].decorator_args == []

    def test_no_arg_decorator_has_empty_args(self, parser: PythonParser) -> None:
        code = "@staticmethod\ndef util():\n    pass\n"
        result = parser.parse(code, "test.py")
        assert result.symbols[0].decorator_args == []

    def test_undecorated_function_has_empty_args(self, parser: PythonParser) -> None:
        code = "def plain():\n    pass\n"
        result = parser.parse(code, "test.py")
        assert result.symbols[0].decorator_args == []

    def test_multiple_decorators_flatten_args(self, parser: PythonParser) -> None:
        code = (
            "@processor('sync')\n"
            "@retry('backoff')\n"
            "def job():\n"
        ...
        result = parser.parse(code, "test.py")
        assert result.symbols[0].decorator_args == ["sync", "backoff"]


class TestDiFieldCapture:
    """The Python parser captures {field: Type} DI maps on the CLASS symbol.

    ``self.x = x`` assignments in ``__init__`` where ``x`` is a typed constructor
    parameter form the Python analogue of NestJS constructor injection.
    """

    def test_init_self_assignment_from_typed_param(self, parser: PythonParser) -> None:
        code = (
            "class UserService:\n"
            "    def __init__(self, prisma: PrismaService, repo: UserRepo, n: int):\n"
            "        self.prisma = prisma\n"
            "        self.repo = repo\n"
            "        self.count = n\n"
            "        self.const = 0\n"
        )
        result = parser.parse(code, "service.py")
        cls = next(s for s in result.symbols if s.kind == "class")
        # n is builtin-typed (skipped); self.const = 0 is not a param (skipped).
        assert cls.di_fields == {"prisma": "PrismaService", "repo": "UserRepo"}

    def test_class_without_init_has_empty_map(self, parser: PythonParser) -> None:
        code = "class Empty:\n    def do_work(self):\n        return 1\n"
