"""Watch mode for Horus — re-indexes on file changes.
...
from horus_source.config.ignore import load_gitignore, should_ignore
from horus_source.config.languages import is_supported
...
from horus_source.core.ingestion.community import process_communities
from horus_source.core.ingestion.coupling import process_coupling
from horus_source.core.ingestion.dead_code import process_dead_code
...
from horus_source.core.ingestion.walker import FileEntry, read_file
...
def _embedding_meta_path(repo_path: Path) -> Path:
    return repo_path / ".horus" / "source" / "meta.json"
...
def ensure_current_embeddings(storage: StorageBackend, repo_path: Path) -> bool:
    """Re-embed the full graph when the stored embedding model is outdated OR incomplete.

    Beyond a model/scheme change, this also recovers an index whose embeddings never finished
    (HOR-375): ``analyze`` writes meta and runs embeddings in a background thread, so an
    interrupted/timed-out index can be left with ``embeddings: 0`` and semantic search silently
    dead. Re-embedding here makes the next host start self-heal such an index.
    """
    ...
    stored_scheme = meta.get("embedding_scheme_version")
    scheme_current = stored_model == _DEFAULT_MODEL and stored_scheme == EMBEDDING_SCHEME_VERSION

    # Incomplete embeddings: an explicit completeness flag is authoritative; fall back to the
    # tell-tale "has symbols but zero embeddings" when an older index predates the flag.
    stats = meta.get("stats") or {}
    embeddings_incomplete = meta.get("embeddings_complete") is False or (
        int(stats.get("embeddings") or 0) == 0 and int(stats.get("symbols") or 0) > 0
    )

    if scheme_current and not embeddings_incomplete:
        return False

    if scheme_current:
        logger.warning(
            "Index embeddings are incomplete (embeddings=%s, symbols=%s) — re-embedding to "
            "restore semantic search (HOR-375)",
            stats.get("embeddings"),
            stats.get("symbols"),
        )
    else:
        logger.info(
            "Embedding scheme changed (model %s->%s, scheme %s->%s), re-embedding all symbols",
            stored_model or "unknown",
            _DEFAULT_MODEL,
            stored_scheme if stored_scheme is not None else "unknown",
            EMBEDDING_SCHEME_VERSION,
        )
